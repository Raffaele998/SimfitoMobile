...
// RIMETTERE A POSTO PRIMA DEL DEPLOY
		...
		foreach($pests as $idpest=>$mpest) {
			...
			if($pestfound) {
				...
				while($r = $db->fetch_assoc()) {
					...
					if ($match) {
						if ($geom) {
							$sqlbuff = bufferareaex($areaid,$r['gid_sito'],0,$buffdataz['tampone'],$buffdataz['contenimento']);
						}
						else {
							$sqlbuff = bufferarea($vps['buffer1'],$vps['buffer2'],$vps['buffer3'],$vps['tiposito'],$vps['pestcode'],$vps['start'],$vps['end'],$r['gid_sito']);
						}
						if ($debug) {
