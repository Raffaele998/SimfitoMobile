	...
	switch($mode){
		...
		if(empty($where0)) $where0="1=1";
			...
			$sql="WITH A AS(
					...
					GROUP BY idscheda
				), D AS ( SELECT idscheda,array_to_string(array_agg(distinct tecnici.nome || ' ' || tecnici.cognome ORDER BY tecnici.nome || ' ' || tecnici.cognome), ', ') as tt
					FROM schede_tecnici 
		...
		if(empty($where0)) $where0="1=1";
			...
			$sql="WITH A AS(
				...
				SELECT report.id AS report_id,campioni.codice, to_char(report.datareport,'YYYY-MM-DD') AS datareport, scheda.idscheda, idschedatecnico, scheda.id_tecnico,
					...
					tecnici2.nome || ' ' || tecnici2.cognome || CASE WHEN array_to_string(array_agg(distinct tecnici.nome || ' ' || tecnici.cognome ORDER BY tecnici.nome || ' ' || tecnici.cognome),', ') <> '' THEN ', ' || array_to_string(array_agg(distinct tecnici.nome || ' ' || tecnici.cognome ORDER BY tecnici.nome || ' ' || tecnici.cognome),', ') ELSE '' END AS tecnici,
		...
		case "aziendeall":
			...
			$sql="SELECT id_azienda, partita_iva,rag_soc,$rup,$vivaio,$fito, array_to_string(array_agg(distinct tipoazienda.descrizione ORDER BY tipoazienda.descrizione), ', ') AS tipo,
